/*
Remove semaphore mutex and add additional semaphores, as needed, so that some
reader could work concurrently with some writer (assuming the buffer contained some
data but was not full – so both reading and writing made sense).
*/
...
int sem_create(int num_semaphores) { /* procedure to create specified number of semaphores */
    int semid;
...
void P(int semid, int index) { /* procedure to perform a P or wait operation on a semaphore of given index */
...
void V(int semid, int index) { /* procedure to perform a V or signal operation on semaphore of given index */
